...
function cylinder(numSlices, numStacks, caps) {
    ...
    for(var j=0; j<stacks; j++) {
       ...
       for(var i=0; i<slices; i++) {
          ...
          var magTangents = Math.sqrt(u[0]*u[0]+u[1]*u[1]+u[2]*u[2]);
          var tang = [u[0]/magTangents, u[1]/magTangents, u[2]/magTangents]
 ...
 function scale(sx, sy, sz){
       for(var i=0; i<cylinderVertexCoordinates.length; i++) {
          ...
          cylinderTangents[i][0] /= sx;
          cylinderTangents[i][1] /= sy;
          cylinderTangents[i][2] /= sz;
       };
 }
 ...
 function rotate( angle, axis) {
       ...
       for(var i=0; i<cylinderVertexCoordinates.length; i++) {
             ...
             for( var k =0 ; k<3; k++) {
                ...
                t[j] += mat[j][k]*cylinderTangents[i][k];
             };
             for( var j =0; j<3; j++) {
                cylinderVertexCoordinates[i][j] = u[j];
                cylinderNormals[i][j] = v[j];
                cylinderTangents[i][j] = t[j];
             };
       };
 }
 ...
 data.TriangleTangents = cylinderTangents;
 data.rotate = rotate;
