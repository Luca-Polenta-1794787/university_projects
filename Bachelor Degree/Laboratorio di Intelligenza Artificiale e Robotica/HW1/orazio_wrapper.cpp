...
void OrazioWrapper::connect(const char *serial_device) {
  ...
  // TODO : 0 Instanciate an orazio client
	...
	PacketStatus controlloSync; // PACKETSTATUS PER IL CONTROLLO DELLA RIUSCITA DELLA SYNC PIU' AVANTI
  //
  if (!_client) {
  ...
  for (int i = 0; i < retries; ++i) {
    ...
    // which fancy function of Orazio client will you have to call?
		controlloSync=OrazioClient_sync(_client, 1);
    //
  }
  printf("done\n");
